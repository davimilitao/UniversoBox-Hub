/**
 * @file PedidosDoDiaV2.jsx
 * @module expedicao
 * @description Cópia de PedidosDoDia v4.3.0 com impressão casada (1 clique):
 *              DANFE + etiqueta impressas em paralelo ao fechar separação;
 *              pending→packed em 1 botão; update otimista (0ms percebido).
 * @version 1.0.0
 * @date 2026-05-11
 * @changelog
 *   1.0.0 — 2026-05-11 — Fork de PedidosDoDia: ModalSeparadoV2 (DANFE+etiqueta paralelas),
 *           confirmarExpedirTudo (pending→packed), printEtiquetaBridge (bridge→fallback),
 *           Bridge ONLINE/OFFLINE no header.
 */
...
// Guard de auto-impressão — chave: orderId → { danfe: bool, label: bool }
...
// ─── Etiqueta via bridge ──────────────────────────────────────────────────────
// Tenta bridge localhost:9191 → ZPL direto na Elgin. Fallback: abre link no browser.
async function printEtiquetaBridge(orderId, onStatus) {
  onStatus?.('Buscando etiqueta…');
  const d = await api(`/api/etiqueta-logistica/${encodeURIComponent(orderId)}`);
  if (!d?.ok) throw new Error(d?.error || 'Erro ao buscar etiqueta');
  ...
  if (d.zpl) {
    try {
      ...
      const br = await fetch('http://localhost:9191/print', {
        ...
        body: JSON.stringify({ zpl: d.zpl }),
      });
      if (br.ok) { onStatus?.('Etiqueta impressa ✓'); return; }
    } catch {} // bridge offline → fallback
  }

  // Fallback: abre link no browser (não lança erro — não bloqueia o fluxo)
  onStatus?.('Etiqueta aberta no browser');
...
// ─── Botão Etiqueta (fallback manual — usado no picked tab) ──────────────────
...
// ─── Modal Separação V2 — DANFE + Etiqueta paralelas + 1 botão ────────────────
function ModalSeparadoV2({ order, proximo, onConfirmarExpedir, onFechar, confirmando }) {
  const [danfeSt,  setDanfeSt]  = useState('idle');
  const [danfeMsg, setDanfeMsg] = useState('');
  const [etiqSt,   setEtiqSt]   = useState('idle');
  const [etiqMsg,  setEtiqMsg]  = useState('');
  ...
  useEffect(() => {
    ...
    async function autoprint() {
      // Guard: já imprimiu nesta sessão
      const done = autoPrintDone.get(order.id) || {};
      const skipDanfe = !!done.danfe;
      const skipEtiq  = !!done.label;

      if (skipDanfe) { setDanfeSt('ok'); setDanfeMsg('DANFE já impressa'); }
      else { setDanfeSt('printing'); }

      if (skipEtiq) { setEtiqSt('ok'); setEtiqMsg('Etiqueta já impressa'); }
      else { setEtiqSt('printing'); }

      if (skipDanfe && skipEtiq) return;

      const jobs = [];
      if (!skipDanfe) {
        jobs.push(
          printDanfe(order.blingNfId, m => { if (!cancelled) setDanfeMsg(m); })
            .then(() => {
      ...
      if (!skipEtiq) {
        jobs.push(
          printEtiquetaBridge(order.id, m => { if (!cancelled) setEtiqMsg(m); })
            .then(() => {
              autoPrintDone.set(order.id, { ...(autoPrintDone.get(order.id)||{}), label: true });
              // Não sobrescreve msg — onStatus já definiu o texto correto (impressa ✓ ou aberta no browser)
              if (!cancelled) setEtiqSt('ok');
            })
            .catch(e => {
              if (!cancelled) { setEtiqSt('err'); setEtiqMsg(e.message); }
            })
        );
      }
  ...
  async function reprintEtiq() {
    if (!order?.id) return;
    setEtiqSt('printing'); setEtiqMsg('');
    try {
      await printEtiquetaBridge(order.id, m => setEtiqMsg(m));
      setEtiqSt('ok'); setEtiqMsg('Etiqueta impressa ✓');
    } catch (e) {
      setEtiqSt('err'); setEtiqMsg(e.message);
    }
  }
      ...
      <div className="bg-slate-900 border border-white/10 rounded-3xl w-full max-w-md max-h-[90vh] overflow-y-auto shadow-2xl">
        ...
        <div className="px-4 pb-3 space-y-2">
          ...
          <PrintStatusRow
            ...
            fallback={danfeSt === 'err' ? () => setDanfeSt('idle') : undefined}
            onReprint={order.blingNfId ? reprintDanfe : undefined}
          />
          <PrintStatusRow
            icon={<Tag size={14} className="shrink-0"/>}
            label="Etiqueta de Transporte"
            st={etiqSt} msg={etiqMsg}
            fallback={etiqSt === 'err' ? () => setEtiqSt('idle') : undefined}
            onReprint={order.id ? reprintEtiq : undefined}
          />
        </div>
        ...
        <div className="flex gap-2 p-4 border-t border-white/5">
          ...
          {/* Habilitado imediatamente — operador não precisa esperar a impressão */}
          <button onClick={() => onConfirmarExpedir(proximo?.id || '')} disabled={!!confirmando}
            ...
            {confirmando
              ? <span className="flex items-center justify-center gap-2"><Loader2 size={14} className="animate-spin"/>Expedindo…</span>
              : <><Truck size={14} className="inline mr-1.5"/>Confirmar e Expedir</>}
          </button>
        </div>
      </div>
    </div>
...
// ─── Modal Expedição (para pedidos já em picked) ───────────────────────────────
...
// ─── Página principal V2 ──────────────────────────────────────────────────────
export default function PedidosDoDiaV2() {
  ...
  // Polling do bridge local
  useEffect(() => {
    const check = () =>
      fetch('http://localhost:9191/status', { signal: AbortSignal.timeout(2000) })
  ...
  // ── Confirmar e Expedir (V2: pending→packed, 1 clique, otimista) ──
  async function confirmarExpedirTudo(proximoId) {
    const id = selOrder?.id;
    if (!id) return;
    setConfirmando(id);

    // Otimista: some da lista instantaneamente
    setOrders(prev => ({ ...prev, pending: prev.pending.filter(o => o.id !== id) }));
    setModalSep(false);
    setSelOrder(null);

    try {
      await api(`/orders/${encodeURIComponent(id)}/status`, { method: 'POST', body: JSON.stringify({ status: 'picked' }) });
      await api(`/orders/${encodeURIComponent(id)}/status`, { method: 'POST', body: JSON.stringify({ status: 'packed' }) });

      if (proximoId) {
        // Usa estado corrente após update otimista
        setOrders(curr => {
          const prox = curr.pending.find(x => x.id === proximoId);
          if (prox) { beep(true); selectOrder(prox); showToast(`✓ Expedido! Próximo: ${proximoId}`, 'ok'); }
          else { beep(true); showToast('Pedido expedido ✓', 'ok'); }
          return curr;
        });
      } else {
        beepListaClear();
      ...
      refreshAll(); // background, sem await
    } catch(e) {
    ...
    <div className="flex-1 min-h-0 flex flex-col overflow-hidden bg-slate-950 text-slate-100">
      ...
      <div className="flex items-center justify-between px-5 py-3 border-b border-white/5 bg-slate-900/80 backdrop-blur-sm flex-shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-black text-base text-slate-100 leading-none">Expedição V2</h1>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg border text-[10px] font-bold
            ...
            title={bridgeOnline ? 'Bridge de impressão online' : 'Bridge offline — etiqueta abrirá no browser'}>
            <Printer size={10}/>
            <span className="hidden sm:inline">{bridgeOnline ? 'Bridge' : 'Bridge'}</span>
            <span>{bridgeOnline ? 'ONLINE' : 'OFFLINE'}</span>
          </div>
          {/* Toggle notificações */}
        ...
        <div className={`flex flex-col overflow-hidden w-full
              ...
              <div className="flex items-center justify-between gap-4 px-5 py-3 border-b border-white/5 bg-slate-900/60 flex-shrink-0">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <button onClick={() => setModalSep(true)} disabled={!progress.allOk || selOrder.status !== 'pending'}
                    className="px-4 py-2 rounded-xl text-sm font-extrabold bg-emerald-600 hover:bg-emerald-500 disabled:opacity-30 disabled:cursor-not-allowed text-white transition-all hover:scale-105 active:scale-95 shadow-lg shadow-emerald-900/30">
                    ✓ Separado
                  </button>
                </div>
              </div>
          ...
          {tab === 'picked' && (selOrder ? (
            <div className="flex flex-col min-h-0 overflow-hidden">
              ...
              <div className="flex-shrink-0">
                <ScannerZone status={scanStatus} hint="bipe DANFE ou apelido para confirmar"
      ...
      {modalSep && (
        <ModalSeparadoV2 order={selOrder} proximo={proximoPedido}
          onConfirmarExpedir={confirmarExpedirTudo} onFechar={() => setModalSep(false)} confirmando={!!confirmando} />
      )}
      {modalExp && (
