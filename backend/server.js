...
// Categorias que representam impostos/tributos (filtradas de despesas operacionais)
...
// Agrega NF-e de saída do Bling para um determinado mês (YYYY-MM)
...
async function blingAgregaMes(mesAno) {
  ...
  // Situações válidas: 2=Emitida DANFE, 5=Autorizada Sem DANFE, 7=Emitida DANFE
  ...
  // Para NFs sem valorTotal no listing, busca detalhes individuais (máx 30)
...
// ── GET /api/margem-v2 ───────────────────────────────────────────────────────
// Agrega dados de margem a partir de fontes reais (Bling NF-e + Firestore)
...
app.get('/api/margem-v2', requireFirebaseAuth, async (req, res, next) => {
  try {
    ...
    // ── 1. Firestore fin_despesas (despesas + impostos por mês) ─────────────
    const deTs  = new Date(de  + '-01T00:00:00');
    const ateTs = new Date(ate + '-01T00:00:00');
    ateTs.setMonth(ateTs.getMonth() + 1); // upper bound exclusivo
    ...
    const despPorMes = {}; // { YYYY-MM: { imposto, despesas } }
    const catsPorMes = {}; // { YYYY-MM: { [categoria]: valor } }
    ...
    // ── 2. Firestore fin_compras (custo de mercadoria por mês) ──────────────
    ...
    const custoPorMes = {}; // { YYYY-MM: number }
    ...
    // ── 3. Firestore fin_margem_mensal (overrides manuais: Shopee, dividendos) ──
    const manualPorMes = {};
    ...
    // ── 5. Google Sheets fallback (histórico — usado se Bling indisponível) ──
    const sheetsPorMes = {};
    ...
    const items = meses.map(mm => {
      ...
      // Receita Bruta: Bling (automático) > Sheets (histórico) > manual
      let receitaBruta, fonte;
      if (receita) {
        const shopeeExtra = Number(manual.receitaShopeeManual || 0);
        receitaBruta = receita.total + shopeeExtra;
        const temDespesas = desp.despesas > 0 || desp.imposto > 0;
        fonte = temDespesas ? 'automatico' : 'parcial';
      } else if (hist) {
...
app.post('/api/margem-v2/importar-historico', requireFirebaseAuth, requireFirebaseRole(['admin']), async (req, res, next) => {
  try {
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      // Não sobrescreve meses já com dados automáticos
